...
class routineMain(object):
    ...
    def __init__(self):
        ...
        self.lastIndex=0
        self.twseDict=self.csvToList('G:/python/oraclet/test/data/twseStockList.csv')
        self.gtsmDict=self.csvToList('G:/python/oraclet/test/data/gtsmStockList.csv')
...
#                 logging.exception("newFindCode")
    ...
    def getEvent(self,content):
        ...
        if '空' in classificationStr:
            ...
            return False
    def newgetEvent(self,content,title):
#         print content
#         print type(content)
        title=title.encode('utf-8').replace('航空','')
#       找title
        if '空' in title:
            return u'賣出'
        elif'多' in title:
            ...
            return False
        classification=content.encode('UTF-8')
        classification=classification.split('2. 分類')
#       2016
        if len(classification)>1:
            try:
#                 classificationStr=classification[1].splitlines()[0].replace('多/空/請益/心得','')
#               新家的 
                classificationStr=classification[1].split('3. 分析/正文')[0].replace('多/空/請益/心得','')

            except IndexError as e:
                logging.exception("2016getEvent")
                return u'買進'
            if '空' in classificationStr:
                return u'賣出'
            elif '請益'in classificationStr or '心得'in classificationStr:
                return False
            elif '多' in classificationStr:
                return u'買進'
            else:
                return u'買進'
#       2015      
        classification=content.encode('utf-8').split('1. 標的')
        if len(classification)>1:
            try:
                classificationStr=classification[1].splitlines()[0].replace('多/空/請益/心得','')
            except IndexError as e:
                logging.exception("2015getEvent")
                return u'買進'
            if '空' in classificationStr:
                return u'賣出'
            elif '多' in classificationStr:
    ...
    def pttBuild(self,d):
        ...
        if codeInfo!=False:
            ...
            oracletDict['event']=self.newgetEvent(mysqlData['content'],mysqlData['title'])
        ...
        if codeInfo!=False:
            try:
                oracletNumber=dbHandler.getOneOraclet(pttSource[0]['author'].split(u'(')[0].encode('utf-8'),
                                  ...
                                  str(codeInfo[0]),
                                  self.newgetEvent(pttSource[0]['content'],pttSource[0]['title']))[0]['number']
            except IndexError as e:
...
#             檢查是否需要更新ptt_source、更新留言庫
    ...
    def parseGos(self,link):
        ...
        try:
            ...
            title = soup.find(id="main-container").contents[1].contents[2].contents[1].string
...
if __name__ == '__main__':
    routine=routineMain()
    routine.main()
#     routine.parseGos('https://www.ptt.cc/bbs/Stock/M.1468650349.A.1CC.html')
#     dbHandler=MySQLHandler()
