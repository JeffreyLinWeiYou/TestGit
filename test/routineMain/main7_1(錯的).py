...
#                 logging.exception("newFindCode")
    ...
    def getEvent(self,content):
        ...
        print content
        try:
            test=content.encode('utf-8').split('2. 分類')
            classificationStr=content.encode('utf-8').split('2. 分類')[1].splitlines()[0].replace('多/空/請益/心得','')
        except IndexError as e:
            try: 
                classificationStr=content.encode('utf-8').split('1. 標的')[1].splitlines()[0].replace('多/空/請益/心得','')
            except IndexError as e:
                return u'買進'
...
#                         需要測試!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        dbHandler.updateOrInsetMessage(temp['id'],pttSource[0]['date'],
                                                       temp['time'],temp['content'].replace(' ',''),
                                                       oracletNumber,temp['status'])
...
#             檢查是否需要更新ptt_source、更新留言庫
    ...
    def parseGos(self,link):
        ...
        try:
            author  = soup.find(id="main-container").contents[1].contents[0].contents[1].string.replace(' ', '').replace("\'",'*').replace("\"",'').replace("\\",'')
            title = soup.find(id="main-container").contents[1].contents[2].contents[1].string.replace("\'",'*').replace("\"",'').replace("\\",'')
...
if __name__ == '__main__':
    routine=routineMain()
#     routine.main()
    routine.parseGos('http://www.ptt.cc/bbs/Stock/M.1451313334.A.489.html')
