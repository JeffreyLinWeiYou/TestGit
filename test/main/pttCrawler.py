...
#                     print "true"
                ...
                Log.i("Error code,date in pttCrawler.crawler: %s" % (str(link)))
                pass
...
#     print sql
    ...
    try:
        ...
        try:
            sql=u"UPDATE ptt_source SET message='%s' where author='%s' AND date='%s' AND title='%s'"%(d['g_message'],d['b_author'],dStr,d['c_title'] )
            cursor.execute(sql)
            db.commit()
            print "update成功!!!!!!!!!!!!!!!!!!!!!!!"
        except:
            Log.i("Error %d- %s in pttCrawler.py_dataToMYsql" % (e.args[0],e.args[1]))
