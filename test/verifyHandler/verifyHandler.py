...
#             print type(self.nowPrice)
    ...
    def foreignRankVerify(self):
        ...
        endDate=self.prdictTime+datetime.timedelta(days = 550)
        if self.type==1:
    ...
    def foreignTargetPriceVerify(self):
        ...
        endDate=self.prdictTime+datetime.timedelta(days = 540)
        if self.type==2:
            getPricrHandler=TwseHandler()
        if self.type==4:
            getPricrHandler=GtsmHandler()
...
#                 print price
    ...
    def pTTRankVerify(self):
        startDate=self.prdictTime
        endDate=self.prdictTime+datetime.timedelta(days = 24)
        if self.type==5:
            getPricrHandler=TwseHandler()
        if self.type==6:
            getPricrHandler=GtsmHandler()
    ...
    def updateToDb(self):
        try:
            if self.result!=None:
                ...
                #type1356
                if self.ROI!=None:
                    sql="UPDATE oraclet SET results1 = %s, roi1 = %s WHERE predict_people = '%s' AND predict_time = '%s' AND predict_targetcode = %d AND event_content='%s'"\
                    ...
                    sql="UPDATE oraclet SET results1 = %s WHERE predict_people = '%s' AND predict_time = '%s' AND predict_targetcode = %d AND event_content='%s'"\
...
#     sql="SELECT * FROM oraclet Where results1 is NULL "
    ...
    for record in results:
        if record[6]<=4:
            occurTime=record[4]+datetime.timedelta(days = 185)
        else:
            occurTime=record[4]+datetime.timedelta(days = 15)
        print "151:",occurTime
        if datetime.datetime.now().date()>occurTime:
            print record
            try:
